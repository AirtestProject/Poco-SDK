            ...
            if (rectTransform)
            {
                ...
                switch (rootCanvas.renderMode)
                {
                    case RenderMode.ScreenSpaceCamera:
                        ...
                        //rootCanvas.scaleFactor会缩放rect在屏幕上的真实的大小,因此乘上。
                        ...
                        break;
                    default:
                        size = new float[] { rect.width * rootCanvas.scaleFactor / (float)Screen.width, rect.height * rootCanvas.scaleFactor / (float)Screen.height };
                        break;
                }
            }
            else
